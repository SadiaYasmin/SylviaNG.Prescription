...
{
    public class GetMyDraftPrescriptionsHandler : IRequestHandler<GetMyDraftPrescriptionsQuery, PrescriptionListResponse>
    {
        ...
        public GetMyDraftPrescriptionsHandler(
        ...
        public async Task<PrescriptionListResponse> Handle(GetMyDraftPrescriptionsQuery query, CancellationToken cancellationToken)
        {
            ...
            var joined =
                ...
                where p.DoctorId == doctorId && p.Status == PrescriptionStatusEnum.Draft
                select new { p, pt };

            if (query.PatientId.HasValue)
                joined = joined.Where(x => x.p.PatientId == query.PatientId.Value);

            if (query.Date.HasValue)
            {
                var start = DateTimeUtility.StartOfDayUtc(query.Date.Value);
                var end = DateTimeUtility.EndOfDayUtc(query.Date.Value);
                joined = joined.Where(x => x.p.SavedAt != null && x.p.SavedAt >= start && x.p.SavedAt < end);
            }
